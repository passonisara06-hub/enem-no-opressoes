...
def calcular_capital_herdado(df: pd.DataFrame) -> dict:
    ...
    sup_mae = df[df["escolaridade_mae"].isin(["Superior completo", "Pós-graduação"])]
    sup_mae_uf = sup_mae.groupby("uf").size().reset_index(name="n_sup_mae")
    total_mae = df.groupby("uf").size().reset_index(name="n_total")
    sintetico = sup_mae_uf.merge(total_mae, on="uf", how="right").fillna(0)
    sintetico["pct_mae_superior"] = sintetico["n_sup_mae"] / sintetico["n_total"] * 100

    # % de mães em profissões liberais/direção (Grupo 5)
    lib_mae = df[df["ocupacao_mae"] == "Grupo 5 — Profissional liberal/direção"]
    ...
    sintetico["pct_mae_liberal"] = sintetico["n_lib_mae"] / sintetico["n_total"] * 100

    # % de pais em trabalho rural/manual (Grupo 1 — mais oprimido)
    rural_pai = df[df["ocupacao_pai"] == "Grupo 1 — Trabalho rural/manual"]
    ...
    sintetico["pct_pai_rural"] = sintetico["n_rural_pai"] / sintetico["n_total"] * 100
